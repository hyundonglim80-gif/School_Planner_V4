...
describe('이름 만들기', () => {
  ...
  it('사진 파일의 기본 이름에 번호가 들어간다', () => {
    expect(photoBaseName(cls, 8, '배유나')).toBe('2026-3-2-8-배유나');
  });

  it('올릴 때는 원본 확장자를 따른다', () => {
    expect(photoFileName(cls, 8, '배유나', 'IMG_0421.JPG')).toBe('2026-3-2-8-배유나.jpg');
    expect(photoFileName(cls, 8, '배유나', 'x.webp')).toBe('2026-3-2-8-배유나.webp');
  });

  it('모르는 확장자는 png로 둔다', () => {
    expect(photoFileName(cls, 8, '배유나', 'scan.heic')).toBe('2026-3-2-8-배유나.png');
    expect(photoFileName(cls, 8, '배유나', '확장자없음')).toBe('2026-3-2-8-배유나.png');
  });
});
...
describe('findPhotoFor', () => {
  it('번호까지 맞는 사진을 고른다', () => {
    const files = [
      { id: 'a', name: '2026-3-2-7-박서진.png' },
      { id: 'b', name: '2026-3-2-8-배유나.png' },
    ];
    expect(findPhotoFor(files, cls, 8, '배유나')).toEqual({
      id: 'b',
      name: '2026-3-2-8-배유나.png',
      exact: true,
    });
  });
  ...
  it('번호가 맞는 것이 있으면 그것을 먼저 고른다', () => {
    const files = [
      ...
      { id: 'new', name: '2026-3-2-9-배유나.png' },
    ];
  ...
  it('확장자만 다른 사진이 여럿이면 늘 같은 것을 고른다', () => {
    const files = [
      { id: 'j', name: '2026-3-2-8-배유나.jpg' },
      { id: 'p', name: '2026-3-2-8-배유나.png' },
    ];
